...
export default async function ClientShareByTokenPage({ params }: Props) {
  ...
  const queueRows = await prisma.job.findMany({
    select: { id: true, status: true, startDate: true, checklist: true },
  });
  ...
  const placeInLine = Math.max(1, activeQueue.findIndex((item) => item.id === job.id) + 1);
    ...
    <main className="min-h-screen bg-stone-50 px-4 py-8 sm:py-12">
      <div className="mx-auto w-full max-w-3xl">
        <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <p className="mt-1 text-sm text-stone-600">Customer: {job.customer.name}</p>
